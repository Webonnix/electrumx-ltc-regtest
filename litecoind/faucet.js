...
const AUTH = '-rpcuser=electrum -rpcpassword=electrum -conf=/home/litecoin/litecoin.conf';
...
schedule.scheduleJob(`*/${UPDATE_INTERVAL_SEC} * * * * *`, function() {
  systemSync(`litecoin-cli -regtest ${AUTH} generate 1`);
});
...
app.post('/faucet/:address', (req, res) => {
  ...
  if (!address) {
    ...
    const result = systemSync(`litecoin-cli -regtest ${AUTH} ${cmd}`);
